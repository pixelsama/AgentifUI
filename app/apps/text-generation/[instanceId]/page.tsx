 ...
 * - 复用workflow的完整架构
 */
export default function TextGenerationPage({ params }: TextGenerationPageProps) {
    ...
    const initializeApp = async () => {
      ...
      try {
        ...
        if (!currentAppMatches) {
          console.log('[文本生成页面] 需要切换应用，从', currentAppId, '到', instanceId)
          
          try {
            await switchToSpecificApp(instanceId)
            console.log('[文本生成页面] 应用切换成功')
          } catch (switchError) {
            console.warn('[文本生成页面] 应用切换失败，但继续加载页面:', switchError)
          }
        } else {
