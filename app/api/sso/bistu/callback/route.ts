...
export async function GET(request: NextRequest) {
  ...
  if (!ticket) {
    ...
    return NextResponse.redirect(
      new URL('/login?error=missing_ticket&message=认证参数缺失，请重新登录', appUrl)
    );
  }
  ...
  try {
    ...
    if (!userInfo.success || !userInfo.employeeNumber) {
      ...
      return NextResponse.redirect(
        new URL(`/login?error=ticket_validation_failed&message=${encodeURIComponent(errorMessage)}`, appUrl)
      );
    }
    ...
    console.log(`Validating employee number: "${employeeNumberStr}" (original type: ${typeof userInfo.employeeNumber}, string length: ${employeeNumberStr.length})`);
    ...
    if (!SSOUserService.validateEmployeeNumber(employeeNumberStr)) {
      ...
      return NextResponse.redirect(
        new URL('/login?error=invalid_employee_number&message=学工号格式不正确', appUrl)
      );
    }
    ...
    if (!user) {
      ...
      console.log('Using hardcoded SSO provider for testing:', ssoProvider.name);
    ...
    return NextResponse.redirect(
      new URL(`/login?error=sso_callback_failed&message=${encodeURIComponent(errorMessage)}`, appUrl)
    );
  }
} 