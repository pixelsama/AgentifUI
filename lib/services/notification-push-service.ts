...
export const pushAgentNotification = (
  ...
  const notification: NotificationWithReadStatus = {
    ...
    title: `Agent "${agentName}" ${success ? '执行完成' : '执行失败'}`,
...
export const pushTokenWarning = (
  ...
  const notification: NotificationWithReadStatus = {
    ...
    title: 'Token使用量警告',
    content: `您的Token使用量已达到${percentage}%，当前使用${currentUsage}/${limit}。请注意控制使用量以避免服务中断。`,
    priority,
    published: true,
    published_at: new Date().toISOString(),
    created_at: new Date().toISOString(),
...
export const pushMaintenanceNotice = (
  ...
  const notification: NotificationWithReadStatus = {
    ...
    title: '系统维护通知',
    content: `系统将于${startTime}进行${maintenanceType}，预计${endTime}结束。维护期间可能出现服务中断，请提前保存工作进度。`,
    priority: 'high',
    published: true,
    published_at: new Date().toISOString(),
    created_at: new Date().toISOString(),
