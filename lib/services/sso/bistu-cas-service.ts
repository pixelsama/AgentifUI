...
export class BistuCASService {
  ...
  generateLoginURL(returnUrl?: string): string {
    try {
      ...
      throw new Error(`Failed to generate CAS login URL: ${error instanceof Error ? error.message : String(error)}`);
    }
  }
  ...
  generateLogoutURL(returnUrl?: string): string {
    try {
      ...
      throw new Error(`Failed to generate CAS logout URL: ${error instanceof Error ? error.message : String(error)}`);
    }
  }
  ...
  async validateTicket(ticket: string, service: string): Promise<BistuUserInfo> {
    ...
    try {
      ...
      console.log(`Validating ticket at: ${validateUrl.replace(/ticket=[^&]+/, 'ticket=***')}`);
  ...
  private parseValidationResponse(xmlText: string): BistuUserInfo {
    try {
      ...
      // --- END COMMENT ---
        ...
        console.error(`CAS authentication failed: ${errorCode} - ${errorMessage}`);
      ...
      throw new Error('Unexpected CAS response format: no success or failure element found');
    } catch (error) {
...
export function createBistuCASService(): BistuCASService {
  ...
  if (!appUrl) {
    throw new Error('NEXT_PUBLIC_APP_URL environment variable is required for SSO configuration');
  }
