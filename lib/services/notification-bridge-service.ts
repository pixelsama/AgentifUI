 ...
 */
import { useNotificationCenter } from '../stores/notification';
import type {
  NotificationAction,
  NotificationType,
...
export class NotificationBridgeService {
  ...
  static showAgentResult(
    ...
    const title = `Agent执行${success ? '完成' : '失败'}`;
    const content = `Agent "${agentName}" ${success ? '成功执行完成' : '执行失败'}${duration ? `，耗时${duration}` : ''}。\n\n详情：${details}`;
  ...
  static showTokenWarning(
    ...
    const title = 'Token使用量警告';
    const content = `您的Token使用量已达到${percentage}%（${currentUsage}/${limit}），请注意控制使用量以避免服务中断。`;
  ...
  private static getArchiveTitle(type: NotificationType): string {
    ...
    switch (type) {
      case 'error':
        return `系统错误 - ${timestamp}`;
      case 'warning':
        return `系统警告 - ${timestamp}`;
      case 'info':
        return `系统信息 - ${timestamp}`;
      case 'success':
        return `操作成功 - ${timestamp}`;
      default:
        return `系统通知 - ${timestamp}`;
    }
  }
}
