    ...
    if (fieldType === 'number') {
      ...
      if (numberConfig.precision !== undefined) {
        ...
        if (decimalPlaces > numberConfig.precision) {
          errors[variable] =
            `${label}小数位数不能超过${numberConfig.precision}位`;
          return;
        }
      }
    }
        ...
        if (file instanceof File) {
          ...
          if (fileConfig.max_file_size_mb) {
            ...
            if (file.size > maxSizeBytes) {
              errors[variable] =
                `${label}中的文件"${file.name}"大小超过${fileConfig.max_file_size_mb}MB`;
              break;
            }
          }
          ...
          if (
            ...
            if (
              ...
              errors[variable] =
                `${label}中的文件"${file.name}"类型不支持，支持的类型：${fileConfig.allowed_file_types.join(', ')}`;
              break;
            }
          }
        }
